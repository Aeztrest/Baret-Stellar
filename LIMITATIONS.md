...
- **Provisioning is best-effort and happens after the first manual approval.** If it fails (RPC error, passphrase no longer cached), that merchant keeps being paid with the wallet's admin key and has **no on-chain cap**; the extension's own bookkeeping still applies. Until a sub-key exists, a stolen extension profile is a stolen wallet.
...
- **Known bug: renewing an expired mandate does not renew the on-chain state.** After the mandate (default 30 days) lapses and the user re-approves, only the local row is extended; the on-chain allowance and the sub-key's signer expiry are not, so payments signed by the old sub-key are rejected. Tracked in `docs/implementation-status.md` §4.
