...
const THEME = {
  ...
  name: "SolYield",
  logo: (
    <div className="w-7 h-7 rounded-lg flex items-center justify-center text-xs font-black" style={{ background: "linear-gradient(135deg,#10b981,#059669)" }}>
      Y
    </div>
  ),
};
...
const POOLS = [
  { name: "Marinade mSOL", apy: "7.2%", tvl: "$284M", risk: "Low", badge: "Audited" },
  { name: "Lido stSOL", apy: "6.8%", tvl: "$142M", risk: "Low", badge: "Verified" },
  { name: "JitoSOL", apy: "8.1%", tvl: "$198M", risk: "Low", badge: "MEV Boost" },
];
...
export default function SolYield() {
  ...
  async function handleStake() {
    ...
    try {
      const __built = await buildScenario(dangerous ? "solyield-warn" : "solyield-safe", walletAddress); const tx = __built.transactionXdr;
      setPreviewTx(tx);
    } catch (e) {
        ...
        <div className="max-w-5xl mx-auto">
          ...
          <div className="grid md:grid-cols-5 gap-8">
            ...
            <div className="md:col-span-2 space-y-3">
              <h2 className="text-sm font-semibold text-white/50 uppercase tracking-wider mb-4">Staking Pools</h2>
              {POOLS.map((p, i) => (
                <button
                  key={p.name}
                  ...
                  style={{
                    background: selectedPool === i ? "rgba(16,185,129,0.1)" : "rgba(255,255,255,0.02)",
                    border: `1px solid ${selectedPool === i ? "rgba(16,185,129,0.3)" : "rgba(255,255,255,0.05)"}`,
                  }}
                >
                  <div className="flex items-center justify-between mb-1.5">
                    <span className="font-semibold text-sm text-white">{p.name}</span>
                    <span className="text-xs px-2 py-0.5 rounded-full" style={{ background: "rgba(16,185,129,0.15)", color: "#10b981" }}>{p.badge}</span>
                  </div>
                  <div className="flex items-center gap-4 text-xs text-white/40">
                    <span>APY <strong className="text-emerald-400">{p.apy}</strong></span>
                    <span>TVL {p.tvl}</span>
              ...
              {dangerous && (
                <div className="p-4 rounded-2xl" style={{ background: "rgba(239,68,68,0.06)", border: "1px solid rgba(239,68,68,0.2)" }}>
                  <div className="flex items-center justify-between mb-1.5">
                    <span className="font-semibold text-sm text-red-300">SuperYield Protocol</span>
                    <span className="text-xs px-2 py-0.5 rounded-full" style={{ background: "rgba(239,68,68,0.15)", color: "#ef4444" }}>UNAUDITED</span>
                  </div>
                  <div className="flex items-center gap-4 text-xs">
                    <span className="text-red-400">APY <strong>48%</strong></span>
                    <span className="text-white/30">TVL $42K</span>
                  </div>
                </div>
              )}
            </div>
            ...
            <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.1 }} className="md:col-span-3">
              <div className="glass rounded-2xl p-6 space-y-5">
                <h2 className="font-bold text-white">Stake XLM</h2>
                ...
                <div className="p-4 rounded-xl" style={{ background: "rgba(255,255,255,0.03)" }}>
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div className="flex gap-1">
                      {["25%", "50%", "MAX"].map((p) => (
                        <button key={p} className="px-2 py-1 rounded-lg text-xs font-semibold text-emerald-400 transition-colors hover:bg-emerald-400/10">
                          {p}
                        </button>
                      ))}
                    </div>
                ...
                <div className="space-y-3 p-4 rounded-xl" style={{ background: "rgba(16,185,129,0.05)", border: "1px solid rgba(16,185,129,0.1)" }}>
                  {[
                    { label: "Staking pool", value: dangerous ? "SuperYield Protocol" : pool.name },
                    { label: "Annual APY", value: dangerous ? "48.0%" : pool.apy },
                    { label: "You receive", value: dangerous ? "sySOL" : "mSOL" },
                    { label: "Estimated yearly", value: `+${estimatedYearly.toFixed(4)} XLM` },
                  ].map(({ label, value }) => (
                    <div key={label} className="flex justify-between text-sm">
                      <span className="text-white/40 flex items-center gap-1">{label} <Info size={11} className="opacity-40" /></span>
                ...
                {success ? (
                  <motion.div initial={{ scale: 0.9 }} animate={{ scale: 1 }} className="w-full py-4 rounded-xl text-center font-bold text-emerald-400" style={{ background: "rgba(16,185,129,0.1)", border: "1px solid rgba(16,185,129,0.25)" }}>
                    ✓ {amount} XLM Staked
                  </motion.div>
                ) : (
                  <button onClick={handleStake} className="w-full py-4 rounded-xl font-bold text-white transition-all hover:brightness-110" style={{ background: "linear-gradient(135deg,#10b981,#059669)" }}>
          ...
          <div className="mt-10 flex justify-center">
            <div className="flex items-center gap-3 px-5 py-3 rounded-2xl" style={{ background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.07)" }}>
              <span className="text-xs text-white/30">Simulate unverified pool</span>
