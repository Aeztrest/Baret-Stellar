...
import { refreshSubKeyAfterApproval } from "../swig/sub-key-lifecycle";
...
const txSignHandler: Handler<"tx.sign"> = async ({
  ...
  try {
    ...
    if (req.x402Mandate) {
      ...
      const before = await readAllowance(req.x402Mandate.allowanceId);
      const mandateWasLive = before ? isMandateLive(before) : false;
      ...
      if (!promoted.ok) {
        ...
        // MV3 service worker being torn down mid-provisioning. Best-effort:
        // it never unwinds the payment (see `swig/sub-key-lifecycle.ts`).
        try {
          await refreshSubKeyAfterApproval({
            allowanceId: req.x402Mandate.allowanceId,
            merchantOrigin: req.x402Mandate.merchantOrigin,
            mandateSeconds,
            mandateWasLive,
          });
        } catch (err) {
          console.warn(`[BARET] sub-key refresh failed for ${req.x402Mandate.merchantOrigin}:`, err);
        }
      }
    }
