...
- **Tipler** (`src/types.ts`): sunucu `Decision`'ının aynası (`AnalysisResult`, `RiskFinding`…). `RiskSeverity` burada `critical` da içerir; sunucu yalnız `low|medium|high` üretir. Bulgu kodları çalışma zamanı dizisi olarak durur: `RISK_FINDING_CODES` sunucunun döndürdüğü kodlar, `CLIENT_FINDING_CODES` istemcinin kendi ürettiği `ANALYZE_UNREACHABLE`, `X402_ASSET_NOT_ALLOWED` ve SEP-10 tanıyıcısının `SEP10_INVALID_CHALLENGE` / `SEP10_ACCOUNT_MISMATCH` / `SEP10_UNVERIFIED_ANCHOR` ve çekme korumasının `SEP6_WITHDRAW_MISMATCH` / `SEP6_WITHDRAW_UNVERIFIED` / `SEP6_WITHDRAW_EXPIRED` kodları (sunucu bunları üretmez). Sunucu kataloğu ile `RISK_FINDING_CODES` `apps/server/test/domain/finding-codes.test.ts` ile kilitlidir; üretici noktalar `satisfies ClientFindingCode` ile tiplenir.
...
CI (`.github/workflows/ci.yml`) bunu `pnpm -r --if-present build → typecheck → lint → test` ile yapar. `lint` yalnızca sunucuda tanımlıdır, `test` sunucu, eklenti, cüzdan ve `packages/{swig-guard,agent-guard,baret-adapter,ext-protocol}`'te. Kontrat işi `cargo fmt --check`, `clippy -D warnings` (yalnız `merchant-spend-policy`; eski `payment-guard` kapsam dışı), `cargo test` ve `wasm32v1-none` derlemesini koşar; ayrıca `docs-consistency` ve `secrets` (`scripts/check-secrets.mjs`) işleri vardır. `testnet-health.yml` haftada bir testnet'te kontratın canlı olduğunu denetler (`pnpm --filter @stellar-thorn/server chain-check`).
