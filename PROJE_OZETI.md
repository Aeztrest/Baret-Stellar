...
```
Kullanıcı imzalamak üzere  →  Baret devreye girer
                               ↓
...
Her imza talebi swig-guard üzerinden Baret analizine girer; riskli işlemler dApp'te değil, cüzdanda bloklanır.
...
- **`analyzeTransaction(cfg, req)`** — Baret `/v1/analyze`'a HTTP istemcisi.
...
- `baret_analyze` — işlem analiz et
- `baret_health` — servis sağlıklı mı
- `baret_list_profiles` — policy profillerini listele
