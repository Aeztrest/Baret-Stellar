...
- **Yapıldı (2026-09-20), dal `feat/anchor-sep10`:**
  - Yeni `apps/extension/src/background/sep/` modülü: `anchors.ts` (allowlist, varsayılan yalnız `tr-mock-anchor.fly.dev`; hostname doğrulaması: port/IP/localhost/yol yok), `toml.ts` (SEP-1 okuyucu: `https://<host>/.well-known/stellar.toml`, 8 sn zaman aşımı, 100 KB sınırı hem `content-length` hem akışta, `redirect: "error"`, 5 dk önbellek, yalnız üst düzey `KEY="değer"` satırları, tablo başlığında durur), `sep10-challenge.ts` (tanıyıcı).
  - `tx.analyzeRequest`, işlemi sunucuya sormadan önce tanıyıcıdan geçirir; challenge biçimli her şeyi (ilk op `… auth` manage_data ya da sequence 0) **kendisi karara bağlar**, sunucuya gitmez. Sonuçlar: bloklu `SEP10_INVALID_CHALLENGE` (critical; `details.rules` bütün ihlalleri listeler), bloklu `SEP10_ACCOUNT_MISMATCH` (high), uyarı `SEP10_UNVERIFIED_ANCHOR` (medium), izinli "X'e giriş, para hareket etmez".
  - Üç yeni kod `CLIENT_FINDING_CODES`'a eklendi (T0.3 drift testi hâlâ yeşil).
  - **SDK `readChallengeTx` kaynağı okundu** (bu görevin "doğrulanmadı" maddesi): sequence, sunucu kaynağı, zaman sınırları (5 dk tolerans), değer uzunluğu (48 bayt), home domain eşleşmesi, sonraki op'ların manage_data + sunucu kaynaklı olması, `web_auth_domain` **varsa** değeri, sunucu imzası zorunlu. **Eksikleri bizim kontrolümüz tamamlıyor:** toml `SIGNING_KEY`'i otorite yapmak (SDK'ya bunu `serverAccountID` olarak veriyoruz), op türlerinin baştan listelenmesi, hesap sahipliği, toml ağ parolası, `web_auth_domain` beklenen değeri toml `WEB_AUTH_ENDPOINT` host'undan almak. SDK `web_auth_domain` op'unun **varlığını** zorunlu tutmaz; biz de tutmuyoruz (spec sunucuya zorunlu kılıyor, istemciye değil).
  - **Plandan sapma (bilerek):** "kullanıcının bulunduğu origin'le uyuşmayan domain" **işaretlenmedi**. Bir cüzdan/dApp kendi origin'inden başka bir anchor'a giriş yaptırır; bu meşru SEP-10 akışıdır. Origin yalnız metinde gösteriliyor ("X, Y'ye giriş yapmanı istiyor").
  - **Plandan sapma (bilerek):** allowlist dışı domain için toml **çekilmez**. XDR'daki domain saldırgan kontrollüdür; çekmek dApp'in eklentiyi keyfi host'lara istek attırmasına izin verir ve sahte anchor kendi toml'uyla kendini doğrulardı. Bunun yerine yapısal kontroller + "doğrulanamadı" uyarısı.
- **Kanıt:** 36 yeni test (`sep/sep10-challenge.test.ts` 21, `sep/toml.test.ts` 12, `messaging/tx-analyze-sep10.test.ts` 3). Fixture'lar `WebAuth.buildChallengeTx` ve elle `TransactionBuilder` ile üretilir (hermetik). Saldırılar: gerçek sequence + payment, sequence 0 + setOptions, birden çok yabancı op türü, ilk op login değil, başka anahtarla imzalı (look-alike), imzasız, süresi dolmuş, sonsuz zaman sınırı, yanlış `web_auth_domain`, başka ağın toml'u, başkasının hesabı. **12 mutasyon gerçekten test kırdı** (11 birim + handler bağlantısının kaldırılması), dosyalar birebir geri yüklendi. Eklenti 144 test, typecheck 0 hata.
- **Canlı duman testi (tek seferlik, geçici test dosyası, silindi):** gerçek tr-mock-anchor'dan alınan gerçek challenge, gerçek toml ile **allow** verdi; aynı challenge başka bir hesabın cüzdanıyla **block**.
- **Kanıt sınırı:** popup'ta görsel gösterim tarayıcıda görülmedi (T2.1 yalnız `AnalyzeResponse` üretir, mevcut popup bunu zaten çiziyor); gerçek bir dApp'ten `signTransaction` ile challenge gönderilmedi (handler testi kuyruğa doğrudan koyuyor). Toml'un canlı CORS başlığı `*` doğrulandı, ama eklentinin service worker'ından gerçek bir tarayıcıda okunması denenmedi (Node `fetch` ile denendi). Allowlist kullanıcı tarafından düzenlenemez (LIMITATIONS'a yazıldı). Fee-bump ile sarılmış challenge tanınmaz, normal analize gider.
...
- **Yapıldı (2026-09-20), dal `feat/anchor-sep-client`:**
  - **Trustline istisnası** (`sep/trustline-exception.ts`): sunucudan önce, işlemdeki **her** `changeTrust` (a) kullanıcının kendi hesabında, (b) sıfır olmayan limitli bir **ekleme**, (c) canonical USDC ya da allowlist'teki bir anchor'ın toml `[[CURRENCIES]]`'inde bildirdiği (kod + issuer birlikte) varlık ise, **yalnız** `blockTrustlineChanges` ve `blockUnlimitedTrustlines` o işlem için kapatılır (policy'nin gerisi aynen uygulanır) ve sign ekranına gerekçe satırı eklenir. Kaldırma, başka hesap, havuz payı, kimsenin kefil olmadığı varlık ve aynı kodlu ama farklı issuer'lı sahte USDC normal kurala tabi kalır. Anchor toml'u yalnız canonical olmayan bir varlık varsa çekilir.
  - `toml.ts` artık `[[CURRENCIES]]` (`code`+`issuer`) okuyor (50 kayıtla sınırlı, diğer tabloların anahtarları sızmaz).
  - Options → Anchors'a **"Account setup"** paneli: hesap fonlu mu, USDC trustline'ı var mı; "Fund with Friendbot" ve "Add USDC trustline" mevcut `wallet.airdrop` / `wallet.addUsdcTrustline`'ı kullanır (yeni RPC yok); trustline düğmesi hesap fonlanana kadar kapalı; anchor'ların akıllı cüzdana değil `G…` hesabına ödediği metinde yazıyor.
- **Bulgu (planda yoktu): varsayılan Balanced profil de etkileniyor.** Balanced `blockUnlimitedTrustlines: true` ve limitsiz `changeTrust` (SDK varsayılanı) bunu tetikliyor; yani istisna yalnız Strict'i değil varsayılan ayarı da ilgilendiriyor. (Balanced `blockTrustlineChanges: false`.)
- **Kanıt:** 19 yeni test (`trustline-exception` 14, handler seviyesi 5, `toml` +2). Sahte fixture'lar: sahte-USDC (başka issuer), anchor'ın bildirdiği koda başka issuer, karışık işlem (biri güvenilir biri değil), kaldırma, başkasının hesabı, işlem kaynağı başkası, havuz payı, okunamayan toml, fee-bump sarmalı. **11 mutasyondan 9'u test kırdı, 2'si eşdeğer mutant çıktı** (havuz payı koruması aynı zamanda tip daraltması ve payın hiçbir güvenilir varlıkla eşleşmemesi sonucu değiştirmiyor; `flush` zaten diğer tabloların anahtarlarını atıyor). Dosyalar birebir geri yüklendi.
- **Gerçek tarayıcı doğrulaması (Chromium 127, canlı Friendbot + Horizon, throwaway testnet hesabı):** Anchors sayfasında "Account is funded" ✗ ve trustline düğmesi **kapalı**; "Fund with Friendbot" ile ✓; "Add USDC trustline" ile "USDC trustline (balance 0.0000000)" ✓.
- **Yan bulgu (T2.3'ün değil, önceden var): kendi işlemlerimiz drift sayılıyor.** Aynı akışta sidebar'da iki **"high" drift alarmı** çıktı ("An unsigned transaction touched your authority"): `wallet.airdrop`, `wallet.transferXlm` ve `wallet.addUsdcTrustline` işlemleri geçmişe yazılmıyor, monitör de geçmişte imzası olmayan her işlemi drift sayıyor. Ayrı görev (T1.7) olarak düzeltilecek.
- **Kanıt sınırı:** istisna gerçek bir dApp'ten gelen `changeTrust` ile tarayıcıda denenmedi (handler testi kuyruğa doğrudan koyuyor); anchor'ın kendi issuer'lı USDC'si (canonical'dan farklı) desteklenmiyor (LIMITATIONS'ta); yatırmanın gerçekten `pending_trust` olmadığı T2.5'te canlı denenecek.
- **Doküman:** `docs/policy-dsl.md`, `docs/extension-architecture.md` §9, `docs/implementation-status.md`, `LIMITATIONS.md`.

#### T2.4 Çekme koruması (ana değer) 🟡 (koruma ✅; Baret'in kendi çekme akışı ⏳)
- **Yapıldı (2026-09-20), 1. kısım: koruma:**
  - Bir işlem, anchor'ın toml `ACCOUNTS` listesindeki bir hesaba değer aktarıyorsa (payment, path payment, `account_merge`, `create_account` hedefleri) "çekme ödemesi" sayılır. Anchor'a **imza anında** sorulur (`GET /sep6/transactions`, kullanıcının SEP-10 JWT'si): dApp'in başlattığı çekme de Baret'in başlattığıyla aynı doğrulanır. İşlem tam olarak **tek** `payment` olmalı: kaynak kullanıcı, hedef/memo (tür+değer)/tutar (stroop)/varlık (kod+issuer) anchor'ınkiyle eşit, anchor'ın durumu `pending_user_transfer_start`. Aksi hâlde **blok** `SEP6_WITHDRAW_MISMATCH` (kritik; her fark listelenir), sunucuya gitmez.
  - **Fail-closed:** giriş yok / anchor'a ulaşılamıyor / anchor'da kayıt yok → blok `SEP6_WITHDRAW_UNVERIFIED` ("Options → Anchors'ta giriş yap, tekrar dene"). Eşleşme → normal analiz + "kontrol edildi" satırı; süresi dolmuş talep → uyarı `SEP6_WITHDRAW_EXPIRED`.
  - `toml.ts` `ACCOUNTS` (tek satırlık dizi, 50 ile sınırlı, yalnız üst düzey) okuyor; `sep/sep6-transactions.ts` yalnız çekmeleri, yalnız karşılaştırılan alanlarla ayrıştırır.
- **Tasarım hatası yakalandı ve düzeltildi (aynı görevde):** ilk sürüm her sıradan ödemede anchor toml'unu çekiyordu; mevcut SEP-10 handler testi bunu yakaladı ("hiçbir anchor'a dokunmaz" artık yanlıştı). Bu, her ödemede anchor'a IP ifşası ve anchor kapalıyken imza öncesi 8 sn'ye varan gecikme demekti. Çözüm: `sep/anchor-accounts.ts` `ACCOUNTS` kümesini `storage.local`'da 24 saat tutar (3 sn zaman aşımıyla yeniler, yenileme başarısızsa eskisini kullanır) → sıradan ödeme **hiç istek atmaz**. Ek olarak, kümede olan bir anchor hesabına ödeme varken toml artık okunamıyorsa ödeme **bloklanır** (eskiden geçerdi).
- **Plandan sapma (bilerek):** planda "yanıttan alınan kayıt" (yerel kayıt) esastı. Anchor'ın kendi `/transactions` kaydını imza anında sormak dApp'ten başlayan çekmeleri de kapsıyor (yerel kayıt yalnız Baret'in başlattıklarını kapsardı) ve kayıt deposu gerektirmiyor. Yerel kayıt (2. kısım) ek olarak "memo yeniden kullanımı" savunması ve Baret'in kendi ödeme akışı için gelecek.
- **Kanıt (1. kısım):** 81 yeni test (`withdraw-guard` 30, `sep6-transactions` 7, `withdraw-verify` 18, `anchor-accounts` 9, handler 6, `toml` +1, `tx-analyze-sep10` düzeltildi; fixture'lar: yanlış hedef, yanlış/eksik/başka türde memo, fazla/eksik tutar, başka varlık kodu, aynı kodlu başka issuer, XLM, path payment, `account_merge`, eklenmiş fazladan op, iki payment, başka kaynaktan payment/işlem, tamamlanmış talep, giriş yok, anchor 500/403, kayıt yok, anchor'ın başka hesabına ödeme, süresi dolmuş). **27 mutasyondan 25'i ilk turda test kırdı; 2 gerçek boşluk (memo türü olmadan değerle eşleme; kalıcı önbelleğin handler'da kullanıldığını doğrulayan test yoktu) bulundu, testler eklendi, yeniden öldürüldü.** Dosyalar birebir geri yüklendi.
- **Kanıt sınırı:** gerçek tr-mock-anchor'a karşı bir çekme ödemesi bu aşamada denenmedi (ödeme için hesapta USDC gerekir: T2.5 yatırma + 2. kısım); yalnız hermetik sahte anchor. Toml okunamazsa çekme ödemesi normal analize düşer (LIMITATIONS'ta).
- **2. kısımda kalanlar:** Baret'in kendi çekme akışı (`anchor.withdraw`: anchor yanıtını doğrula — `account_id` toml `ACCOUNTS`'unda mı, `/transaction` ile tutarlı mı; IndexedDB v5 `sep_transactions` kaydı, beklenen ödeme **bir kez** yazılır ve değiştirilmez; `anchor.payWithdrawal`: klasik ödemeyi kayıttan kur, koruma fonksiyonuyla kendini doğrula, imzala, gönder), Options arayüzü, memo yeniden kullanımı için yerel talimatlar.
- **Orijinal plan metni:**
